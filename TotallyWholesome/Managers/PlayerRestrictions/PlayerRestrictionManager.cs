...
namespace TotallyWholesome.Managers.PlayerRestrictions;
...
public class PlayerRestrictionManager : ITWManager
{
    ...
    public void ApplyForcedMute(bool mute)
    {
        Main.Instance.MainThreadQueue.Enqueue(() =>
        {
            if (mute && !Patches.IsForceMuted && ConfigManager.Instance.IsActive(AccessType.AllowForceMute, LeadManager.Instance.MasterId))
            {
                ...
                if (!ConfigManager.Instance.IsActive(AccessType.EnableMuffledMode, LeadManager.Instance.MasterId))
                    AudioManagement.SetMicrophoneActive(false);

                Patches.IsForceMuted = true;
                Patches.IsMuffled = ConfigManager.Instance.IsActive(AccessType.EnableMuffledMode, LeadManager.Instance.MasterId);
            }
            ...
            AvatarParameterManager.Instance.TrySetParameter("TWGag", mute && ConfigManager.Instance.IsActive(AccessType.AllowForceMute, LeadManager.Instance.MasterId) ? 1 : 0);
        });
    }
